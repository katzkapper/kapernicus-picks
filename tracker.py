...
def update_result(game, date, result):
    ...
    updated = 0
    for p in picks:
        if (p.get("game","").lower() == game.lower() and
                p.get("date","") == date and
...
def calculate_stats(picks):
    ...
    return {
        ...
        "r20_record":    f"{r20_wins}-"
                         f"{len(r20_picks)-r20_wins}",
        "r31_record":    f"{r31_wins}-"
                         f"{len(r31_picks)-r31_wins}",
    }
...
def serve_tracker(port=8080):
    ...
    print("Share your Replit URL with .repl.co "
          "at the end for public access")
